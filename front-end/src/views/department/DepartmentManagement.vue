<template>
  <div class="department-management">
    ...
    <el-row :gutter="20" style="margin-bottom: 20px;">
      <el-col :span="12">
        <el-input v-model="searchForm.keyword" placeholder="搜索部门名称" style="width: 300px; margin-right: 10px;">
          <template #append>
      ...
      <el-table-column prop="status" label="状态" width="100">
        <template #default="scope">
          <el-tag :type="scope.row.status === 'NORMAL' ? 'success' : 'danger'">
            {{ scope.row.status === 'NORMAL' ? '正常' : '已删除' }}
          </el-tag>
        </template>
      </el-table-column>
    ...
    >
      <el-form :model="form" label-width="80px">
        ...
        <el-form-item label="上级部门">
          <el-select v-model="form.parentId" placeholder="选择上级部门">
...
const getDepartments = async () => {
  try {
    ...
    if (response.success) {
      departmentList.value = response.records || []
...
const handleSubmit = async () => {
  try {
    ...
    if (dialogType.value === 'add') {
      response = await createDepartment(form)
    } else {
      response = await updateDepartment(form.id!, form)
    }
    if (response.success) {
...
const handleBatchSubmit = async () => {
  try {
    const response = await batchOperateDepartments({
